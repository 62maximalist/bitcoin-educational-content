...
Panduan langkah demi langkah ini nunjukin ke kamu cara nulis pesan di Blockchain Bitcoin pakai Wallet Electrum. Operasi ini menggunakan instruksi OP_RETURN buat masukin teks ke dalam transaksi, yang bisa dilihat publik di Blockchain. Ikuti langkah-langkah sederhana berikut biar prosesnya berhasil.
...
- Koneksi internet.
- Beberapa satoshi (Sats) atau bitcoin (BTC) di Wallet milikmu untuk menutupi jumlah transaksi dan biaya.
...
2. Unduh versi yang sesuai dengan sistem operasi milikmu (Windows, macOS, Linux).
...
2. Pilih Buat Wallet baru atau Pulihkan Wallet yang sudah ada jika kamu sudah memiliki frasa seed (frasa pemulihan).
3. Ikuti petunjuk untuk mengkonfigurasi Wallet:
 - Untuk Wallet yang baru, catatlah kalimat seednya dan simpanlah di tempat yang aman (kertas, brankas, dll.).
 - Untuk Wallet yang sudah ada, masukkan seedphrase untuk memulihkannya.
4. Tetapkan kata sandi untuk mengamankan Wallet.
...
Kalau perlu, kirim dulu BTC atau Sats buat ngisi Wallet kamu. Caranya, buka tab "Terima" lalu klik "Buat Permintaan":
...
- hindari memasukkan konten yang mungkin kamu sesali

*Ruang Blockchain dan bitcoin milikmu sangat berharga, gunakanlah dengan bijak dan dengan niat*

Mengonversi pesan kamu ke heksadesimal:

- Kamu bisa menggunakan [alat online] (https://www.rapidtables.com/convert/number/ascii-to-hex.html), tetapi berhati-hatilah untuk tidak memproses data sensitif di sana (meskipun, pada prinsipnya, informasi yang dimaksudkan untuk publikasi pada Blockchain Bitcoin melalui OP_RETURN tidak menimbulkan masalah kerahasiaan);
...
- **Tujuan Address**: Bitcoin Address yang valid. Ici, `bc1q879cv4p5q6s9537orange3zss33d3turzad8`. Ini bisa berupa Address kamu sendiri, kalau kamu ingin mengembalikan dana yang ditransfer ke kamu sendiri;
...
⚠️ **Perhatian**: Penting banget buat nunjukin `0` setelah OP_RETURN, karena opcode ini menandai skrip sebagai tidak valid, bikin output jadi nggak bisa dipakai lagi. Node jaringan bakal ngapus output ini dari set UTXO mereka. Kalau kamu masukin nilai selain `0`, nilainya bakal hilang permanen: Bitcoin kamu dianggap terbakar. Makanya, selalu masukin `0` bareng OP_RETURN buat nyimpen data yang diinginkan, tapi tanpa nyambungin dana, biar nggak hilang.
...
Klik OK dan konfirmasikan transaksi dengan kata sandimu. Jendela konfirmasi akan muncul.
...
- Pastikan output yang berisi OP_RETURN sama dengan nol, kalau nggak, bitcoin kamu akan hilang secara permanen.
...
- Pesan singkat: Entri OP_RETURN dibatasi hingga 80 byte. Pikirkan pesan kamu dengan tepat.
